...
$('.mainContentBgDiv').height($('.mainContentDiv').height());
...
$('#star1, #star2, #star3, #star4, #star5').click(function () {
  let star_id = $(this).attr('id');
  let star_num = star_id.charAt(star_id.length - 1);
  let num_of_stars = 5;
  for (let i = 0; i < num_of_stars; i++) {
    let star_id_string = '#star' + (i + 1).toString();
    if (i < parseInt(star_num)) {
      console.log(star_id_string);
      $(star_id_string).addClass("bi-star-fill");
      $(star_id_string).css("color", "#ffdf00");
      $(star_id_string).removeClass("bi-star");
    } else {
      $(star_id_string).addClass("bi-star");
      $(star_id_string).removeClass("bi-star-fill");
      $(star_id_string).css("color", "#f8f9fa");;
    }
  }
...
function report_order() {
  let reason = prompt("Please describe the reason for the report", "");
  if (reason !== null) {
    $('#report_area').val(reason);
    $('#report_button').click();
  }
}
