...
<body>
    ...
    <div class="container-fluid p-3 mainContentDiv">
        <div class="container-fluid bg-dark text-light rounded mainContentBgDiv">
            ...
            $total_pages = client_get_total_pages($_SESSION['client']);
            if (isset($_GET['page_num']) && $_GET['page_num'] > 0 && $_GET['page_num'] <= $total_pages) {
                $current_page = $_GET['page_num'];
            } else {
                $current_page = 1;
            }
            client_show_orders($_SESSION['client'], ($current_page - 1));
            ?>
            <div>
                <form action="order.php" method="post">
                    <button type="submit" name="new_order" class="btn btn-success btn-outline-success rounded-circle border-1 border-light buttonAddOrder"><i class="bi bi-plus fs-4"></i></button>
                </form>
                <?php
                echo
    ...
    <!-- own js -->
    <script src="../../js/client/userMain.js"></script>
